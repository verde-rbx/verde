...
function Store.set(_key: string, _value: unknown)
	local state: Fusion.Value<typeof(_value)> | nil = Store.get(_key)
	if state then
		state:set(_value)
	else
...
function Store.get<T>(_key: string): Fusion.Value<T> | nil
	local state = Store._store[_key]
	if not state then
		state = Fusion.Value(nil)
...
function Store.getValue<T>(_key: string): T | nil
